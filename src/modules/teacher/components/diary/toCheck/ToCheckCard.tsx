...
interface ToCheckCardProps {
    item: IConnectionGroup
}

const ToCheckCard:FC<ToCheckCardProps> = ({item}) => {
    ...
    return (
        <>
        {item?.students?.filter((item) => item.isDone).map((student) => 
        <Paper key={item.id} sx={{padding:'10px 10px', mt:'20px'}}>
            <Grid container>
                <Grid item xs={1}>
                    <Image src={'/images/person.svg'} width={30} height={30} alt='iconbook' style={{marginTop:'5px'}}/>
                </Grid>
                <Grid item xs={11}>
