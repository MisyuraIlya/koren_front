...
const SecondCard = ({item}: {item: ICourse}) => {
    ...
    return (
        <Box sx={{width:'90%'}}>
            <Card sx={{backgroundColor:'#0172E8', color:'white', borderRadius:'4px', cursor:'pointer'}} onClick={() => {setClickedLvl3(!clickedLvl3); router.push(`/teacher/courses/${lvl1}/${lvl2}/${item.id}/0/0`)}}>
                <Typography variant='body1' fontWeight={900} textAlign={'center'} sx={{padding:'10px 0px'}}>{item.name}</Typography>
            </Card>
            {clickedLvl3 && item?.children?.map((item, key) =>
                <Box key={key}>
                    <ThirdCard item={item}/>
                </Box>
            )}
